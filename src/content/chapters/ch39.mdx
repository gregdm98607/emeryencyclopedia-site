...
What follows is organized as a working researcher would encounter it: from the foundational paper maps that anchored the twentieth century, through the digital portals that now dominate, to the apps that put a topographic map in your pocket on the day of the hike. Where a tool is paired with another chapter of this encyclopedia — geology with Ch02 (see <CrossRef chapter={2} />), mining with Ch17, hydrology with Ch03, travel logistics with Ch36 (see <CrossRef chapter={36} />) — the cross-reference is noted so a reader can spiral outward from a single question into the spatial dimension of the answer.

<TriviaCallout
  text="About four out of every five acres in Emery County belong to the federal government — and the lines dividing BLM from National Forest from state Trust Land from a private homestead were drawn on maps before anyone walked them on the ground. In a place this checkerboarded, a single backcountry route can cross three kinds of land ownership in a quarter mile. That's why a phone with a good land-ownership map isn't a luxury here — it's how you know whose land your boots are actually on."
/>
...
<FamilyActivity
  title="Look Up the Land"
  ageRange="Ages 10 and up"
  duration="20 minutes"
  description="Use the free parcels.utah.gov viewer to click a piece of Emery County ground that means something to you — a family farm, a grandparent's old lot, the land under your house — and read off its owner, acreage, and tax-roll value; then try a piece of the San Rafael Swell and see whose name comes up."
  materials={["A computer or phone with internet"]}
/>
...
The relationship between the statewide UGRC parcels layer and the county's own plat record is best understood as one of generalization. The UGRC layer is sufficient for spatial analysis at the statewide or county scale; the County Recorder's plat is the authoritative source for legal boundary, easement, road dedication, and subdivision detail. Cross-reference Ch37 (Archives, Museums & Libraries) for the broader context of the county courthouse complex as a documentary repository (see <CrossRef chapter={37} />), and Ch38 (Historic Registries) for the cluster of registered buildings around the courthouse itself.
...
The **National Park Service** does not directly manage land within Emery County's boundary, but the county is shaped by adjacent NPS units (Capitol Reef NP to the south, Glen Canyon NRA to the far southeast) and by the Old Spanish National Historic Trail, designated in 2002 and jointly managed by NPS and BLM (see <CrossRef chapter={38} />). The NPS open data portal is at https://public-nps.opendata.arcgis.com/.
...
Cross-reference Ch03 (Hydrology & Springs) for the substantive hydrology of the county (see <CrossRef chapter={3} />); Ch39's role is to point to the spatial layer that lets a reader put any spring or perennial reach on the map.
...
<DataTable
  caption="Table 39-D — NAIP Coverage Years for Utah"
  note="Source: §39.10; Ch39_sources.md."
  id="table-39-d"
>

| Coverage year | Resolution |
|---|---|
| 2003 | not specified in chapter/sources |
| 2004 | not specified in chapter/sources |
| 2006 | not specified in chapter/sources |
| 2009 | not specified in chapter/sources |
| 2011 | not specified in chapter/sources |
| 2014 | not specified in chapter/sources |
| 2016 | not specified in chapter/sources |
| 2018 | 0.6 m (60 cm) — resolution standard since this year, per §39.10 |
| 2021 | 0.6 m (per 2018-forward standard stated in §39.10) |
| 2024 | 0.6 m; most recent statewide recollection (per §39.10) |
...
<FamilyActivity
  title="The Aerial Time Machine"
  ageRange="Ages 12 and up"
  duration="45 minutes (an afternoon with the field trip)"
  description="Download free NAIP aerial imagery of your part of Emery County for two different years — say 2009 and 2024 — load them as a swipe comparison in QGIS, and hunt for what changed: a new reservoir, a new house on SR-10, a dirt road pushed into the Swell. Then drive out and photograph one spot you found."
  materials={["A computer (free QGIS, or any NAIP viewer)", "A camera and a short drive for the field bonus"]}
/>
...
<FigureImage
  src="/images/chapters/ch39_figure_citizen-workflow.png"
  alt="Vertical flow diagram of a five-step citizen researcher workflow: frame the question, gather the layers, overlay in QGIS, field-verify, share the result"
  caption="Figure 39-A. A citizen researcher's workflow for putting a question on a map. Source: Ch39 §39.12, 'Apps, Tools, and Citizen Mapping.'"
  credit="Original diagram for this work"
/>
...
<DataTable
  caption="Table 39-C — Recommended Mapping Tools Comparison"
  note="Source: §39.12."
  id="table-39-c"
>

| Tool | Platform | Price | Offline support | Key strength for Emery County |
|---|---|---|---|---|
| Avenza Maps | Mobile (iOS/Android) | Free tier (3 maps); paid removes limit | Yes, cached GeoPDF | Official BLM/USFS/NPS GeoPDFs in one app |
| Gaia GPS | Mobile + web | Subscription | Yes, paid tiers | Layered basemaps: USGS, USFS, OSM, satellite, terrain |
| OnX Backcountry / Off-Road / Hunt | Mobile | Subscription | Yes | Strongest land-ownership overlay for checkerboard country |
| CalTopo | Web + mobile | Free tier (generous); paid team tiers | Yes, paid tiers | Map printing; precise cross-reference to USGS quads |
| QGIS | Desktop (Windows, macOS, Linux, Android) | Free, open-source | N/A — local project data | Full layered GIS analysis; accessible alternative to ArcGIS Pro |
| QField | Mobile companion to QGIS | Free, open-source | Yes | Field data collection from a QGIS project |

</DataTable>

<DataTable
  caption="Table 39-B — Authoritative vs. User-Contributed Sources by Use Case"
  note="Source: §39.5, §39.12."
  id="table-39-b"
>

| Use case | Authoritative source | User-contributed alternative | Caveat |
|---|---|---|---|
| Land ownership | UGRC Land Ownership layer; BLM Surface Management | OnX / Gaia GPS ownership overlays | Third-party overlays mirror the authoritative layer; they are not the record of title |
| Legal parcel boundary | County Recorder plat maps (§39.5) | UGRC statewide parcels layer | The UGRC layer is a generalized statewide mirror; the Recorder's plat controls in a dispute |
| Motorized / trail access | BLM Travel Management Plan maps; Manti-La Sal MVUM (§39.7) | AllTrails; OpenStreetMap | AllTrails trail descriptions have at times routed users across private or SITLA land (§39.12) |
| Historic quad editions | USGS topoView / HTMC (§39.2) | — | No user-contributed equivalent exists; topoView is the primary historical archive |
| Geology and mine locations | UGS Geologic Map Portal; Abandoned Coal Mine Maps (§39.6) | Gaia GPS community layers | Community layers are not cross-checked against UGS survey data |

</DataTable>

<DataTable
  caption="Table 39-A — Master Directory of Emery County Mapping Resources"
  note="Source: §§39.2–39.12; Ch39_sources.md."
  id="table-39-a"
>

| Resource | Publisher | Format | Primary use |
|---|---|---|---|
| USGS topoView / HTMC | U.S. Geological Survey | Web / GeoPDF | Historic 7.5-minute topo quads, 1884–2006 |
| National Map Downloader | U.S. Geological Survey | Web tool | Topo, elevation, hydrography, NAIP downloads |
| USGS LidarExplorer | U.S. Geological Survey | Web tool | 3DEP lidar point clouds and DEMs |
| UGRC Open Data Portal (SGID) | Utah Geospatial Resource Center | Web / shapefile / GeoJSON | Statewide GIS layers, 27 categories |
| parcels.utah.gov | Utah Geospatial Resource Center | Web viewer | Statewide parcel lookup |
| Emery County IS/GIS | Emery County | Web / office | County-generated maps |
| Emery County Recorder — Plat Maps | Emery County Recorder | Web viewer / PDF | Authoritative legal parcel boundaries |
| UGS Geologic Map Portal | Utah Geological Survey | Web / PDF | Geologic quadrangle maps |
| UGS Abandoned Coal Mine Maps | Utah Geological Survey | Web / interactive | Historic coal-mine surface and subsurface workings |
| BLM Utah GIS Data Library | Bureau of Land Management | Web / file geodatabase | Surface management, Travel Management Areas, leases |
| Manti-La Sal NF GIS / MVUM | U.S. Forest Service | Web / GeoPDF | Motor Vehicle Use Maps |
| SITLA GIS Portal | Utah Trust Lands Administration | Web / PDF | Trust-land ownership and lease records |
| USGS WBD / 3DHP | U.S. Geological Survey | Web / download | Watershed boundaries, hydrography |
| USDA Data Gateway (NAIP) | USDA Farm Service Agency | Web request | Aerial imagery, 2003–2024 |
| David Rumsey Map Collection | David Rumsey / Stanford University | Web archive | Nineteenth-century historic maps |
| Library of Congress Geography & Map Division | Library of Congress | Web archive | Historic Utah maps |
| QGIS | QGIS Development Team | Free desktop software | Layered GIS analysis |
| OpenStreetMap / Utah Trails Stewardship Initiative | OpenStreetMap Foundation / OSM US | Web, citizen-editable | Trail and road corrections |
...
- [UGRC Open Data Portal](https://opendata.gis.utah.gov/)
- [Emery County Recorder — Plat Map Portal](https://emerycounty.com/home/offices/recorder/plat-map/)
- [Utah Geological Survey — Geologic Map Portal](https://geology.utah.gov/portfolio-item/geologic-map-portal/)
- [BLM Utah GIS Data Library](https://www.blm.gov/services/geospatial/GISData/utah)
- [David Rumsey Map Collection](https://www.davidrumsey.com/)
- [OpenStreetMap US — Utah Trails Stewardship Initiative](https://openstreetmap.us/our-work/trails/)
